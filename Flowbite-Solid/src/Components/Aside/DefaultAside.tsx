...
export interface MenuInterface {
    text: string,
    route: string,
    condition: string | ((url: string) => boolean),
    children?: undefined | MenuInterface[],
}

const DefaultAside: Component = () => {
    const helper = useHelper();
    const [show, setShow] = createSignal<boolean>(false);
    helper.page.subscribe('DefaultAside', (all, changedData) => {
        if ('ShowDefaultAside' in changedData)
            setShow(all?.ShowDefaultAside === '1');
    });
    onCleanup(() => {
        helper.page.unsubscribe('DefaultAside')
    })

    helper.url.subscribe('test', (current, previous) => {
        Log.debug('url subscribe log', {current, previous});
    })
    const generateItems = (all: MenuInterface[] | undefined | null,idCounter = '1',depth=0) => {
        if (!all || (Array.isArray(all) && all.length === 0))
            return <></>;
        let tmp = 0;
        return (
                <For each={all}>{(m)=>
                    <Show when={Array.isArray(m?.children) && m.children.length > 0} fallback={
                        <li class={`pl-${depth*3}`}>
                            <A href={Routes[m.route]}
                        ...
                        <li class={`pl-${depth*3}`}>
                            <button type="button"
                                    ...
                                    aria-controls={`${tmp}|${idCounter}`} data-collapse-toggle={`${tmp}|${idCounter}`}
                                    aria-expanded="false">
                                {m.text}
                                <svg aria-hidden="true" class="w-6 h-6 right-0" fill="currentColor" viewBox="0 0 20 20"
                            ...
                            <ul id={`${tmp++}|${idCounter}`} class= "py-2 space-y-2 hidden">{generateItems(m.children,idCounter+'-1',depth+1)}</ul>
                        </li>
                    </Show>
    ...
    return (
        <>
                <aside id="default-sidebar"
                       class={`${show() ? '' : 'hidden'} fixed top-0 left-0 z-40 w-64 h-screen transition-transform sm:translate-x-0 transform-none`}
                       aria-label="Sidenav" aria-modal="true" role="dialog">
                    <div
                    ...
                    <div
                        ...
                        <div
                            ...
                            id="language-dropdown" data-popper-placement="bottom" data-popper-reference-hidden=""
                            data-popper-escaped=""
                ...
                <div drawer-backdrop="" onClick={() => helper.page.set('ShowDefaultAside', '0')}
                     class={`${show() ? '' : 'hidden'} bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-30`}></div>
        </>
    );
}
